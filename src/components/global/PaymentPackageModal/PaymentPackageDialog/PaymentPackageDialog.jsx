...
import { APICreateOrderPremium, APIGetAllSubscriptionPackage } from '../../../../api/axios/Subscription';
import RedeemHeader from '../../../pages/RedeemGiftCode/RedeemHeader/RedeemHeader';
import LoadingAnimation from '../../LoadingAnimation/LoadingAnimation';
import PaymentWaitingDialog from '../PaymentWaitingDialog/PaymentWaitingDialog';

function PaymentPackageDialog({ open, setOpen }) {
    const [loadingPackage, setLoadingPackage] = useState(false)
    const avatar = window.sessionStorage.getItem("avatar");
    const username = window.sessionStorage.getItem("username");
    const [paymentWaitingDialog, setPaymentWaitingDialog] = useState(false)
    ...
    const onSubmit = async () => {
        if (subscriptionPackage?.activeObject === null || pay?.activeBrand === null) {
            ...
            const resPayment = await APICreateOrderPremium(pay.activeBrand.id, subscriptionPackage.activeObject.id);
            if(resPayment?.status === 200) {
                handlePaypalNavigate(resPayment.data)
                setPaymentWaitingDialog(true)
                setOpen(false) // Close dialog payment package
            } else {
    ...
    const handlePaypalNavigate = (paymentUrl) => {
    ...
    return (
        <React.Fragment>
            ...
            <PaymentWaitingDialog open={paymentWaitingDialog} setOpen={setPaymentWaitingDialog} />
        </React.Fragment>
    )
}

export default PaymentPackageDialog