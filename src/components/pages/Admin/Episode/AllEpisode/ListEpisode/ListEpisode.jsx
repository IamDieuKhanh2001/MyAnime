...
import { APIGetEpisodeBySeriesId, APIGetMovieSeries } from "../../../../../../api/axios/adminAPI";
...
export default function ListEpisode() {
    ...
    return (
        <>
            {
                selectedEp ?
                    <Dialog key={selectedEp.id} maxWidth="md" fullWidth={true} open={openUpdateEp} onClose={() => { setOpenUpdateEp(false) }}>
                        <UpdateEpisode series={selectedViewSeries} hideDiaglogUpdate={hideDiaglogUpdate} key={selectedEp.id} ep={selectedEp} />
                    </Dialog> : null
            }
                        ...
                        <div key={series.id} className="allEpisodeLine">
                            <div className="allEpisodeContent">
                                    ...
                                    <div className="episodeItem">
                                        {
                                            episodes?.map(ep => {
                                                return (<button
                                                    onClick={()=>{setSelectedEp(ep);setOpenUpdateEp(true)}}
                                                     className="btn episodeButton">{ep.title}</button>)
                                            })
                                        }
                                        {
                                ...
                                <button
                                    ...
                                    onClick={() => { getEpisodeBySeriesId(series.id); setSelectedViewSeries(series) }}
                                ></button>
                            </div>
